...
BOOL FASTCALL GPIOBUS::Init(mode_e mode)
{
	...
	if (drvfd >= 0) {
		...
		ioctl(drvfd, IOCTL_INIT, baseaddr);
	...
	// システムタイマ
	SysTimer::Init((DWORD *)map + SYST_OFFSET / sizeof(DWORD));
...
{
#ifndef BAREMETAL
	if (baseaddr == 0x3f000000) {
		// RPI2以降はコアタイマーIRQを無効する
...
//
//---------------------------------------------------------------------------
volatile DWORD* SysTimer::baseaddr;
...
//
//---------------------------------------------------------------------------
void FASTCALL SysTimer::Init(DWORD *addr)
{
	baseaddr = addr;
}
